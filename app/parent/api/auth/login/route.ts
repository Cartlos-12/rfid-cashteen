...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      if (parents.length === 0) return NextResponse.json({ success: false, message: "Invalid credentials" }, { status: 401 });

      const parent = parents[0];
      const validPassword = await bcrypt.compare(password, parent.password);
      if (!validPassword) return NextResponse.json({ success: false, message: "Invalid credentials" }, { status: 401 });

      // Generate short-lived JWT (1 hour)
      const token = jwt.sign({ studentId: parent.id }, JWT_SECRET, { expiresIn: "1h" });
