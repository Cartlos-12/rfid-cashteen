...
export default function ReceiptPage({ data, onClose }: { data: ReceiptData; onClose: () => void }) {
  ...
  useEffect(() => {
    // Try printing, but NEVER show any error popup — keep silent
    const sendToPrinter = async () => {
      try {
        ...
        if (!response.ok) {
          console.warn("Printer server not available or failed.");
          return;
        }
        ...
        console.warn("Printer offline — skipping automatic print.");
      }
    };
            ...
            <div className="modal-body">
              ...
              <div className="mb-2">
                ...
                {data.oldBalance !== undefined && data.newBalance !== undefined && (
                  <>
                    <p className="mb-1">
                      <strong>Old Balance:</strong> ₱{data.oldBalance.toFixed(2)}
                    </p>
                    <p className="mb-1">
                      <strong>New Balance:</strong> ₱{data.newBalance.toFixed(2)}
                    </p>
                  </>
                )}
              </div>
